...
- (instancetype)initWithData:(NSDictionary *)info;
{
    if (self = [super init]){
        ...
        NSNumber* streamID =[info valueForKey:@"streamId"];
        _streamId = [NSString stringWithFormat:@"%ld",(long)streamID.integerValue];
        self.data = info;
        if (!_headerSizeArray){
            _headerSizeArray = [[NSMutableArray alloc] initWithArray:@[@(300), @(180), @(135), @(135),@(135), @(135), @(100), @(100)]];
...
- (void)onClose
{
    ...
    [[KMCNetwork sharedInst] leaveMultiRoom:@{@"roomName":roomName, @"userId":uuid,@"roomId":roomId} successBlk:^(NSDictionary *data) {
...
- (void)onQueryChatList:(NSTimer *)timer
{
    ...
    if (roomName && roomName.length > 0){
        ...
        [[KMCNetwork sharedInst] fetchMultiChatListWithRoomName:roomName roomId:roomId successBlk:^(NSDictionary *data) {
...
- (void)authSuccess:(KMCAgoraARTC *)sender
{
    //join
    if (sender){
        [sender joinChannel:_streamId uid:0];
    }
}

- (void)authFailure:(AuthorizeError)iErrorCode
{
    [self toast:[NSString stringWithFormat:@"鉴权出错了, 错误码:%@", @(iErrorCode)]];
}
...
- (void)toast:(NSString *)msg
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        hud.labelText = msg;
        //hud.yOffset = CGPointMake(0.f, MBProgressMaxOffset);
